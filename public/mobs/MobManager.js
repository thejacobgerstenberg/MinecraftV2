...
// Integrated build: the real game block registry IS the fallback (the old
// standalone blocksAdapter shim was deleted at integration time — callers
// may still inject a custom getBlockDef via opts, e.g. for tests).
import { getBlockDef as fallbackGetBlockDef } from '../src/blocks/blocks.js';
...
export class MobManager extends EventTarget {
  ...
  spawn(archetype, pos) {
    ...
    const mob = {
      ...
      // Physics AABB dimensions (from ARCHETYPE_CONFIG) — stamped on the mob
      // so integration code (player melee raycast vs mob hitboxes) can build
      // the hitbox without reaching into this module's config table.
      halfWidth: config.halfWidth ?? 0.3,

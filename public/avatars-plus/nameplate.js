...
export function makeNameplate(opts = {}) {
  ...
  // ADOPTION FIX (Loomfall): clamp() returns its LOW bound for non-finite
  // input, so `clamp(undefined, lo, hi) || default` yielded lo (0.3 m wide
  // plates that fully faded at 4 m) instead of the documented defaults.
  // Apply the documented defaults explicitly when the option is absent.
  const width = Number.isFinite(o.width) ? clamp(o.width, 0.3, 8) : DEFAULT_W;
  const height = width * (CANVAS_H / CANVAS_W);
  const maxDistance = Number.isFinite(o.maxDistance) ? clamp(o.maxDistance, 4, 512) : 48;

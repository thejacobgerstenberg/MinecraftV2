...
export class PlayerStack {
  ...
  constructor(opts = {}) {
    ...
    this.social = new SocialLayer({
      ...
      canvas: o.canvas,
      // ADOPTION ADDITION (Loomfall): a real directed-whisper transport
      // (net.sendWhisper) — makes /w actually private via the server relay.
      sendWhisper: o.sendWhisper,
    });
    safe(() => this.social.enable());

    // --- third-person camera rig -----------------------------------------
    this.tpCamera = new ThirdPersonCamera(this.camera, { isSolid: this.isSolid });
    // ADOPTION FIX (Loomfall): the rig defaults to "third-back"; the game
    // starts first-person (F5 cycles first -> third-back -> third-front).
    safe(() => this.tpCamera.setViewMode("first"));
   ...
   * ADOPTION FIX (Loomfall): the decoded descriptor also replaces the `skin`
   * string, so PresenceStore derives the correct roster swatch from it
   * (buildSkinFromDescriptor expects a descriptor object, not the wire string).
   * @param {object} msg
   * @returns {object}
   */
  _peerMsg(msg) {
    ...
    if (enc) {
      ...
      if (d) return { ...msg, descriptor: d, skin: d };
    }
    return msg;
  }
  ...
  setNet(net) {
    ...
    // ADOPTION FIX (Loomfall): presence gets the same _peerMsg-normalized
    // records as the renderer, so roster swatches derive from real descriptors.
    const onState = (w) => safe(() => {
      const welcome = w || {};
      const list = Array.isArray(welcome.peers) ? welcome.peers : [];
      const norm = list.map((p) => this._peerMsg(p));
      this.social.presence.ingestWelcome({ ...welcome, peers: norm });
      for (const p of norm) this.peers.upsert(p);
      this.peers.setDimension(this._dim());
    });

    const onPeerJoin = (m) => safe(() => {
      const norm = this._peerMsg(m);
      this.peers.upsert(norm);
      this.peers.setDimension(this._dim());
      this.social.presence.upsertPeer(norm);
    });
    ...
    // ADOPTION FIX (Loomfall): intentional closes (quit to title) stay silent;
    // an unexpected drop raises both the toast and a system chat line (parity
    // with the pre-adoption main.js onDisconnect handler).
    const onDisconnect = (info) => safe(() => {
      if (info && info.intentional) return;
      this.social.feed && this.social.feed.note && this.social.feed.note("warn", "Disconnected");
      this.chat && this.chat.addMessage &&
        this.chat.addMessage({ system: true, text: "Disconnected from server." });
    });
   ...
   *
   * ADOPTION FIXES (Loomfall):
   *  - the camera position is fed to PeerAvatarsPlus.update so nameplates
   *    actually distance-fade (the facade originally dropped that argument);
   *  - the third-person rig only drives the camera in a third-person mode and
   *    while NOT spectating — in "first" mode the game's own Player/Controls
   *    keep exclusive camera ownership (identical to pre-adoption behavior),
   *    and while spectating the SpectatorCamera owns the camera.
   * @param {number} dt seconds since last frame.
   */
  update(dt) {
    safe(() => this.peers.update(dt, this.camera ? this.camera.position : undefined));
    safe(() => this.social.update(dt));
    const spectating = !!(this.social && this.social.isSpectating && this.social.isSpectating());
    if (this._getLocalFeet && this._getLocalLook && !spectating &&
        this.tpCamera && this.tpCamera.viewMode !== "first") {
      safe(() => {

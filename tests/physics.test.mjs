...
const GRAVITY = -32; // spec: -0.08 b/tick^2 x 400 (20 Hz)
...
// ── 2. Jump arc: leaves ground, peak ~1.17 blocks, then re-lands ──────────
// Spec jump velocity 8.4 b/s (0.42 b/tick), gravity -32 b/s^2: analytic apex
// 8.4^2/64 ≈ 1.10; the discrete integrator (jump tick sets vy AFTER gravity,
// as Player.update does) overshoots to ≈ 1.17.
{
  const world = { getBlock: (x, y, z) => (y < 10 ? 3 : 0) };
  const pos = { x: 0.2, y: 10, z: 0.2 };
  const vel = { x: 0, y: 8.4, z: 0 };
  const dt = 1 / 60;
  let leftGround = false, apex = pos.y, landed = false;
  for (let i = 0; i < 300 && !landed; i++) {
    const res = moveAndCollide(world, { pos, size: SIZE }, vel, dt);
    pos.y = res.position.y;
    vel.y += GRAVITY * dt; // gravity for the NEXT frame (mirrors the jump tick)
...
// a 0.6-wide box walking +x must stop at pos.x = 5 - 0.6 = 4.4 (± skin).
{
  ...
  const vel = { x: 4.317, y: 0, z: 0 };
  const dt = 1 / 60;
  let collidedX = false;
  for (let i = 0; i < 120; i++) {
    vel.x = vel.x === 0 ? 4.317 : vel.x; // keep pushing into the wall
    vel.y += GRAVITY * dt;
    const res = moveAndCollide(world, { pos, size: SIZE }, vel, dt);
    pos.x = res.position.x; pos.y = res.position.y; pos.z = res.position.z;
...
// ── 4. High speed no-tunneling: vx=78.4 (terminal), dt=0.05, 1-block wall ─
// From x=3.5 a single unsubstepped step would land the box at [7.42, 8.02],
// entirely past the wall cell x=5. Substepping must catch it at 4.4.
{
  const world = { getBlock: (x, y, z) => (y < 10 ? 3 : (x === 5 ? 3 : 0)) };
  const pos = { x: 3.5, y: 10.001, z: 0.2 };
  const vel = { x: 78.4, y: 0, z: 0 };
  const res = moveAndCollide(world, { pos, size: SIZE }, vel, 0.05);
  check('tunnel: blocked by 1-block wall at 78.4 b/s (terminal)', res.collided.x === true);
...
// ── 6. Sneak: 0.3x walk speed (~1.295 b/s, spec SPEED_SNEAK), sprint-cancel, eye drop ─
{
  ...
  check('sneak: walk speed is 0.3x (~1.295 b/s)', approx(hvel, 4.317 * 0.3, 0.02), `hvel=${hvel}`);
  check('sneak: 1s of sneaking covers ~1.295 blocks', disp > 1.2 && disp < 1.4, `disp=${disp}`);
  ...
  check('sneak: sprint is canceled while sneaking', approx(hvel2, 4.317 * 0.3, 0.02), `hvel=${hvel2}`);
}
...
// ── 9. Sprint-jump: ~1.2x forward impulse on the jump tick ────────────────
{
  ...
  const before = Math.hypot(p.velocity.x, p.velocity.z); // ~5.612 (sprint)
...
// ── 10. Terminal velocity: fall speed capped at 78.4 b/s (spec -3.92 b/tick) ─
{
  ...
  p.onGround = false;
  let minVy = 0;
  for (let i = 0; i < 240; i++) { // 4s free fall (uncapped would hit ~128 b/s)
    p.update(DT, mkInput(), 0);
    minVy = Math.min(minVy, p.velocity.y);
  }
  check('terminal velocity: fall speed never exceeds 78.4 b/s', minVy >= -78.4 - 1e-6, `minVy=${minVy}`);
  check('terminal velocity: terminal speed is actually reached', minVy <= -78.3, `minVy=${minVy}`);
}

// ── 11. Walk / sprint speeds pin to the spec values ───────────────────────
{
  const p = mkPlayer(FLAT);
  for (let i = 0; i < 120; i++) p.update(DT, mkInput({ forward: true }), 0);
  const walk = Math.hypot(p.velocity.x, p.velocity.z);
  check('walk speed: 4.317 b/s (spec SPEED_WALK)', approx(walk, 4.317), `walk=${walk}`);
  const q = mkPlayer(FLAT);
  for (let i = 0; i < 120; i++) q.update(DT, mkInput({ forward: true, sprint: true }), 0);
  const sprint = Math.hypot(q.velocity.x, q.velocity.z);
  check('sprint speed: ~5.612 b/s (spec SPEED_SPRINT)', approx(sprint, 5.612, 0.002), `sprint=${sprint}`);
}

// ── 12. AUTO-STEP (STEP_HEIGHT 0.6) ────────────────────────────────────────
// Palette is full blocks only, so a ground-level obstacle top is always
// feet+1.0 > 0.6 and must NOT be stepped. The 0.6 boundary is exercised via
// a stub world that overrides getBlockDef with a partial-height cell
// (collisionHeight 0.6) — the exact spec boundary steps; sneaking never steps.
{
  const LOW = 99; // stub id: solid, collision top 0.6 above the cell floor
  const lowDef = { solid: true, liquid: false, collisionHeight: 0.6 };
  // Flat floor below y=10; obstacle occupying cells z <= -1 at y=10.
  const mkStepWorld = (obstacleId) => ({
    getBlock: (x, y, z) => (y < 10 ? 3 : (z <= -1 && y === 10 ? obstacleId : 0)),
    getBlockDef: (id) => (id === LOW ? lowDef : getBlockDef(id)),
  });

  // Walking forward (yaw 0 -> -z) into a FULL block: 1.0 > 0.6, must NOT step.
  const full = mkPlayer(mkStepWorld(3));
  for (let i = 0; i < 120; i++) full.update(DT, mkInput({ forward: true }), 0);
  check('auto-step: full 1-block obstacle is NOT stepped (1.0 > 0.6)',
    approx(full.position.y, 10, 0.01) && full.position.z >= -0.4 - 1e-6,
    `y=${full.position.y} z=${full.position.z}`);

  // Walking into a 0.6-high obstacle: exact boundary, steps up smoothly.
  const low = mkPlayer(mkStepWorld(LOW));
  for (let i = 0; i < 120; i++) low.update(DT, mkInput({ forward: true }), 0);
  check('auto-step: 0.6-high obstacle IS stepped onto (exact boundary)',
    approx(low.position.y, 10.6, 0.01) && low.position.z < -0.5,
    `y=${low.position.y} z=${low.position.z}`);
  check('auto-step: onGround preserved across the step', low.onGround === true);

  // Sneaking into the same 0.6 obstacle: stepping is disabled while sneaking.
  const sneak = mkPlayer(mkStepWorld(LOW));
  for (let i = 0; i < 240; i++) sneak.update(DT, mkInput({ forward: true, sneak: true }), 0);
  check('auto-step: sneaking never steps up (edge-guard semantics preserved)',
    approx(sneak.position.y, 10, 0.01) && sneak.position.z >= -0.4 - 1e-6,
    `y=${sneak.position.y} z=${sneak.position.z}`);
}

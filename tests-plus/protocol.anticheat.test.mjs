...
//   EDIT_RATE 20/s · CHAT_RATE 3/2000ms (err 'chat_rate') · MAX_REACH 7
...
// at the (~0,80,0) join spawn (well within MAX_REACH=7). Used as a positive
    ...
    for (const y of [79, 80, 81]) {
      for (let x = -5; x <= 5 && cells.length < 60; x++) {
        for (let z = -5; z <= 5 && cells.length < 60; z++) {
          if (reach(x, y, z) <= 6.2 && !(x === 0 && z === 0 && y === 80)) cells.push([x, y, z]);
        }
      }
    }

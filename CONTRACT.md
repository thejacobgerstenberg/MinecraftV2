...
- `public/content/` — canonical Loomfall content data (served statically, fetched at runtime): `achievements.json`, `naming.json`, `tips.json`, `splashes.json`, `deathmessages.json`, `GAME_GUIDE.md`, plus reference lore/data (`bestiary`, `boss`, `items`, `books`, `dialogue`, `structures`, `lore/onboarding/ending` docs) for future stages.
...
- `public/src/systems/achievements.js` — `initAchievements({bus, audio, caps})` engine: wires ONLY triggers whose events exist (26/60 today), per-world localStorage persistence, toast UI + `achievement` fanfare, pause-menu Achievements screen. `stats() -> {total, wired}`.
...
- `public/src/systems/deathmessages.js` — `loadDeathMessages()`, `deathMessageFor(cause, playerName)` ({player}-templated canon lines; causes `mob:<canonicalId>`, `void_unravel`, ...).

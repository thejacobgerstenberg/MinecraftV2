 ...
 *
 * Implements the frozen loader API in LOADER_API.md exactly:
...
export class ContentPack {
  ...
  async _doLoad() {
    const jobs = [
      ...
      ...UX_FILES.map((name) => ({ name, dir: this._uxUrl })),
    ];
    const results = await Promise.allSettled(
      jobs.map((job) => this._readJson(job.dir, job.name))
    );
  ...
  async _readJson(dir, name) {
    if (this._files) {
      const raw = this._files[name];
      if (raw == null) throw new Error(`not in files map`);
      return typeof raw === 'string' ? JSON.parse(raw) : raw;
    }
    const path = `${String(dir).replace(/\/+$/, '')}/${name}`;
    if (this._fetchFn) {
